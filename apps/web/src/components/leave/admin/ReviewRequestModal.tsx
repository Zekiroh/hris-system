    ...
    <div className="pro-modal-overlay">
      ...
      >
            ...
            <div className="space-y-3 text-sm">
              <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
                <span className="flex shrink-0 items-center gap-2 text-gray-500 font-medium">
                  <LeaveTypeIcon
                    className={`w-4 h-4 shrink-0 ${getLeaveTypeColor(request.leaveType)}`}
                  />
                  Leave Type:
                </span>
              ...
              <div className="flex flex-col gap-1 sm:flex-row sm:items-start sm:justify-between">
                ...
                <span className="font-semibold text-gray-800 break-words sm:text-right">
                  {formatLeaveDate(request.startDate)} to {formatLeaveDate(request.endDate)} ({request.days} days)
                </span>
              </div>
        ...
        <div className="pro-modal-footer flex-wrap gap-2">
          <button onClick={onClose} className="btn btn-secondary flex-1 sm:flex-none">
            Cancel
          </button>
