...
const getOvertimeHours = (record: PayrollRecordDto) => {
    const overtimeItem = record.items.find((item) => item.description.toLowerCase().includes('overtime'));
    if (!overtimeItem) return 0;

    const match = overtimeItem.description.match(/(\d+(?:\.\d+)?)\s*(?:hr|hrs|hour|hours)/i);
    return match ? Number(match[1]) : 0;
};

const getLateAbsentDays = (record: PayrollRecordDto) => {
    const lateAbsentItem = record.items.find((item) => {
        const description = item.description.toLowerCase();
        return description.includes('late') || description.includes('absent') || description.includes('absence') || description.includes('undertime');
    });

    if (!lateAbsentItem) return 0;

    const match = lateAbsentItem.description.match(/(\d+(?:\.\d+)?)\s*(?:day|days)/i);
...
const mapPayrollRecordToPayslip = (
    ...
    const deductionItems = record.items
    ...
    return {
        ...
        totalGross: formatPeso(record.grossPay),
        totalDeductions: formatPeso(record.totalDeductions),
        netTakeHome: formatPeso(record.netPay),
        lateAbsentDays: getLateAbsentDays(record),
        overtimeHours: getOvertimeHours(record),
    };
};
...
const MyPaySlips = () => {
    ...
    const ytd2026 = useMemo(() => payslipData.filter(d => d.period.includes('2026')), [payslipData]);

    const ytdGross = useMemo(() => ytd2026.reduce((s, d) => s + parsePeso(d.totalGross), 0), [ytd2026]);

    const ytdDeductions = useMemo(() => ytd2026.reduce((s, d) => s + parsePeso(d.totalDeductions), 0), [ytd2026]);

    const ytdNet = useMemo(() => ytd2026.reduce((s, d) => s + parsePeso(d.netTakeHome), 0), [ytd2026]);

    const ytdSSS = useMemo(() => ytd2026.reduce((s, d) => s + (d.deductionItems.find(x => x.label.includes('SSS')) ? parsePeso(d.deductionItems.find(x => x.label.includes('SSS'))!.amount) : 0), 0), [ytd2026]);

    const ytdPhilHealth = useMemo(() => ytd2026.reduce((s, d) => s + (d.deductionItems.find(x => x.label.includes('PhilHealth')) ? parsePeso(d.deductionItems.find(x => x.label.includes('PhilHealth'))!.amount) : 0), 0), [ytd2026]);

    const ytdPagIBIG = useMemo(() => ytd2026.reduce((s, d) => s + (d.deductionItems.find(x => x.label.includes('Pag-IBIG')) ? parsePeso(d.deductionItems.find(x => x.label.includes('Pag-IBIG'))!.amount) : 0), 0), [ytd2026]);

    const ytdTax = useMemo(() => ytd2026.reduce((s, d) => s + (d.deductionItems.find(x => x.label.includes('Tax')) ? parsePeso(d.deductionItems.find(x => x.label.includes('Tax'))!.amount) : 0), 0), [ytd2026]);
    ...
    const latest = payslipData[0] ?? emptyPayslip;
